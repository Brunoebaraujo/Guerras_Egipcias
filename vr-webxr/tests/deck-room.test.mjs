...
test('pyramid antechamber exposes spatial deck controls and starts with two chosen decks',()=>{
 ...
 const deckCards=room.controls.filter(mesh=>mesh.userData.id.startsWith('deck-card-'));assert.equal(deckCards.length,24);assert.equal(deckCards.filter(mesh=>mesh.visible).length,24);assert.equal(new Set(deckCards.map(mesh=>mesh.userData.deckSide)).size,2);
 ...
 assert.ok([...filters,...presets].every(mesh=>mesh.position.z> -1.5));assert.ok(room.detail.position.y>1.7&&room.detail.position.y<2);
