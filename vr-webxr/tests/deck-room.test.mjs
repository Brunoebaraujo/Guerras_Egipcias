...
test('pyramid antechamber exposes spatial deck controls and starts with two chosen decks',()=>{
 ...
 const cards=room.controls.filter(mesh=>mesh.userData.id.startsWith('card-'));assert.equal(cards.length,16);assert.ok(cards.every(mesh=>mesh.geometry.parameters.width<.4));assert.equal(new Set(cards.slice(0,8).map(mesh=>mesh.position.x)).size,8);
 ...
 assert.ok([...filters,...presets].every(mesh=>mesh.position.z> -1.5));assert.ok(room.detail.position.y>=2.2);
 ...
 room.activate('filter-1',filters[1]);assert.equal(room.filter,'CUSTO 0');room.activate('filter-0',filters[0]);const before=room.getDecks(),selectedCard=cards.find(card=>before[0].includes(card.userData.cardKey));assert.ok(selectedCard);room.activate(selectedCard.userData.id,selectedCard);assert.deepEqual(room.getDecks(),before);assert.equal(room.detailKey,selectedCard.userData.cardKey);const remove=room.controls.find(mesh=>mesh.userData.id==='detail-remove'),add=room.controls.find(mesh=>mesh.userData.id==='detail-add');room.activate('detail-remove',remove);assert.equal(room.getDecks()[0].includes(room.detailKey),false);room.activate('detail-add',add);assert.equal(room.getDecks()[0].includes(room.detailKey),true);
 const preset=room.controls.find(mesh=>mesh.userData.id==='preset-0');room.activate('preset-0',preset);const botTab=room.controls.find(mesh=>mesh.userData.id==='side-1');room.activate('side-1',botTab);room.activate('preset-0',preset);assert.deepEqual(room.getDecks()[1],PRESETS.Padrão);
