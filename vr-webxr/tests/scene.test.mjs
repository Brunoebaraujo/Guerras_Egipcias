...
globalThis.document={createElement(){return {width:0,height:0,getContext(){return {fillRect(){},clearRect(){},strokeRect(){},fillText(){},drawImage(){},beginPath(){},arc(){},fill(){},stroke(){},measureText(t){return {width:t.length*16};}};}};}};
...
test('floating lane totals and left-hand round panel stay in their reading positions',()=>{
 ...
 assert.equal(w.laneLabels.length,6);assert.ok(w.laneLabels.every(label=>label.mesh.rotation.x===0&&label.mesh.position.y===.18));
 assert.equal(w.energy.mesh.parent,w.hand);assert.ok(w.energy.mesh.position.y<0);
