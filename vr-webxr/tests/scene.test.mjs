...
test('scene has 3 × 2 × 4 raycastable slots and every hand card can be picked from desktop',()=>{
  ...
  for(const card of w.cards){const target=card.mesh.getWorldPosition(new THREE.Vector3());target.x+=.001;ray.set(origin,target.sub(origin).normalize());const hit=ray.intersectObjects(w.cards.map(c=>c.mesh));assert.equal(hit[0].object.userData.id,card.definition.id);}
