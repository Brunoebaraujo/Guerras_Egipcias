...
import {effectivePresets,initialDecks,readSavedDecks} from './deck-builder.js?v=1.1.0';
...
export function createDeckRoom(scene,{onStart,storage=globalThis.localStorage}={}){
 ...
 const ui=new THREE.Group();ui.position.set(0,1.18,-1.48);ui.rotation.x=-.48;stage.add(ui);box(ui,0,0,-.045,3.42,2.28,.09,0x5b4a36);
 const title=textPanel(ui,'none',3.18,.31,{pixelsX:1024,pixelsY:160});title.mesh.position.set(0,.88,.015);title.mesh.raycast=()=>{};
 const sideButtons=[textPanel(ui,'side-0',1.45,.22),textPanel(ui,'side-1',1.45,.22)];sideButtons[0].mesh.position.set(-.78,.62,.02);sideButtons[1].mesh.position.set(.78,.62,.02);
 const choiceButtons=Array.from({length:8},(_,i)=>{const p=textPanel(ui,'choice-'+i,.72,.17);p.mesh.position.set(-1.17+(i%4)*.78,.39-Math.floor(i/4)*.2,.02);return p;});
 const choicePageButton=textPanel(ui,'choice-page',.34,.37);choicePageButton.mesh.position.set(1.49,.29,.02);
 const cards=Array.from({length:6},(_,i)=>{const p=textPanel(ui,'card-'+i,.94,.38);p.mesh.position.set(-1.05+(i%3)*1.05,-.08-Math.floor(i/3)*.43,.025);return p;});
 const previous=textPanel(ui,'previous',.42,.18),pageLabel=textPanel(ui,'none',.62,.18),next=textPanel(ui,'next',.42,.18);previous.mesh.position.set(-.54,-.78,.02);pageLabel.mesh.position.set(0,-.78,.02);pageLabel.mesh.raycast=()=>{};next.mesh.position.set(.54,-.78,.02);
 const random=textPanel(ui,'random',.68,.19),clear=textPanel(ui,'clear',.55,.19),start=textPanel(ui,'start',1.48,.19);random.mesh.position.set(-1.22,-1.01,.02);clear.mesh.position.set(-.55,-1.01,.02);start.mesh.position.set(.72,-1.01,.02);
 ...
 let decks=initialDecks(storage?.getItem?.(STORAGE_KEY)),side=0,page=0,choicePage=0,message='Escolha os dois decks e pressione INICIAR PARTIDA.';
 ...
 function paintCard(panel,card,index){const {ctx,canvas,texture}=panel;ctx.clearRect(0,0,canvas.width,canvas.height);if(!card){panel.mesh.visible=false;return;}panel.mesh.visible=true;const selected=decks[side].includes(card.key);ctx.fillStyle=selected?'#273f3b':'#112631';roundRect(ctx,4,4,canvas.width-8,canvas.height-8,18);ctx.fill();ctx.strokeStyle=selected?'#71eee2':'#b99a5d';ctx.lineWidth=selected?10:6;ctx.stroke();ctx.textAlign='left';ctx.textBaseline='middle';ctx.fillStyle='#fff0c5';fit(ctx,card.name,canvas.width-75,31,17);ctx.fillText(card.name,22,52,canvas.width-74);ctx.fillStyle='#7ee5ed';ctx.font='bold 25px sans-serif';ctx.fillText(`${card.cost} ENERGIA · ${card.power} PODER`,22,99);ctx.fillStyle='#c9c5ba';ctx.font='21px sans-serif';ctx.fillText(card.type,22,139,canvas.width-45);ctx.fillStyle=selected?'#71eee2':'#d5ba7a';ctx.font='bold 37px sans-serif';ctx.textAlign='center';ctx.fillText(selected?String(decks[side].indexOf(card.key)+1):'＋',canvas.width-38,48);texture.needsUpdate=true;panel.mesh.userData.cardIndex=index;}
 function render(){
  const maxPage=Math.max(0,Math.ceil(catalog.length/6)-1);page=Math.min(page,maxPage);const active=side===0?'SEU DECK':'DECK DO BOT';
  ...
  const choiceStart=choicePage*8;choiceButtons.forEach((button,i)=>{const choice=choices[choiceStart+i];paintButton(button,choice?choice.name:'',{small:true});button.mesh.userData.choiceIndex=choiceStart+i;});paintButton(choicePageButton,choices.length>8?`DECKS\n${choicePage+1}/${Math.ceil(choices.length/8)}`:'PRESETS',{small:true});
  const offset=page*6;cards.forEach((panel,i)=>paintCard(panel,catalog[offset+i],offset+i));paintButton(previous,'◀',{enabled:page>0});paintButton(next,'▶',{enabled:page<maxPage});paintButton(pageLabel,`${page+1} / ${maxPage+1}`,{small:true});
 ...
 function activate(id,object){
  ...
  if(id==='previous'&&page>0){page--;render();return;}if(id==='next'&&page<Math.ceil(catalog.length/6)-1){page++;render();return;}
