...
export function createGauntlet(side){
 const root=new THREE.Group(),sign=side==='left'?-1:1,renderOrder=side==='right'?360:330;
 ...
 for(const [color,parts] of batches){const inst=new THREE.InstancedMesh(new THREE.BoxGeometry(1,1,1),new THREE.MeshStandardMaterial({color,roughness:.48,metalness:.5,transparent:true,depthTest:false,depthWrite:false}),parts.length);inst.renderOrder=renderOrder;parts.forEach(([x,y,z,w,h,d,rz],i)=>{temp.position.set(x,y,z);temp.rotation.set(0,0,rz);temp.scale.set(w,h,d);temp.updateMatrix();inst.setMatrixAt(i,temp.matrix);});root.add(inst);}
 const ring=new THREE.Mesh(new THREE.TorusGeometry(.009,.003,4,10),new THREE.MeshStandardMaterial({color:0xd9bc76,metalness:.65,roughness:.4,transparent:true,depthTest:false,depthWrite:false}));ring.position.set(0,.026,.056);ring.renderOrder=renderOrder;root.add(ring);
 return root;
}
