...
export function createCardView(table,hand,slots){
 ...
 return {cards,sync,arrangeHand,setMounted,attachSelected,clearSelected,material,handMaterial,powerMaterial};
}
...
export function createInspection(parent,{position=[0,.72,-.08],controlKind='inspection-action'}={}){
 const group=new THREE.Group();parent.add(group);group.position.set(...position);group.visible=false;
 ...
 const mesh=new THREE.Mesh(new THREE.PlaneGeometry(.72,1.08),material);mesh.renderOrder=200;mesh.visible=false;group.add(mesh);
 const frame=typeof Image==='undefined'?null:new Image(),images=new Map();let definition=null;
 function wrap(text,x,y,maxWidth,lineHeight,maxY){let line='';for(const word of String(text||'Sem efeito.').split(' ')){const next=line+word+' ';if(line&&ctx.measureText(next).width>maxWidth){ctx.fillText(line.trim(),x,y,maxWidth);y+=lineHeight;line=word+' ';}else line=next;if(y>maxY)break;}if(y<=maxY)ctx.fillText(line.trim(),x,y,maxWidth);}
 function loadArt(key){if(!key||images.has(key)||typeof Image==='undefined')return;const img=new Image();images.set(key,img);img.onload=paint;img.src=new URL('../card-art/'+(key==='token-cabra'?'cabra-nilo':key)+'.webp',import.meta.url).href;}
 function disk(x,y,value,color){ctx.fillStyle='#0b202a';ctx.beginPath();ctx.arc(x,y,74,0,Math.PI*2);ctx.fill();ctx.strokeStyle=color;ctx.lineWidth=14;ctx.stroke();ctx.fillStyle='#fff4c8';ctx.textAlign='center';ctx.textBaseline='middle';ctx.font='bold 88px Georgia';ctx.fillText(String(value),x,y+4);}
 function paint(){const d=definition;if(!d)return;ctx.clearRect(0,0,1024,1536);ctx.fillStyle='#102633';ctx.fillRect(0,0,1024,1536);
  if(d.hidden){ctx.fillStyle='#162c38';ctx.fillRect(70,70,884,1396);ctx.strokeStyle='#d4ad62';ctx.lineWidth=24;ctx.strokeRect(82,82,860,1372);ctx.textAlign='center';ctx.fillStyle='#dfba70';ctx.font='240px Georgia';ctx.fillText('☥',512,800);ctx.font='bold 54px Georgia';ctx.fillText('CARTA OCULTA',512,1050);}
  else {const art=images.get(d.key);if(art?.complete&&art.naturalWidth)ctx.drawImage(art,154,190,714,588);else{ctx.fillStyle='#263c45';ctx.fillRect(154,190,714,588);}if(frame?.complete&&frame.naturalWidth)ctx.drawImage(frame,0,0,1024,1536);else{ctx.strokeStyle='#d4ad62';ctx.lineWidth=22;ctx.strokeRect(24,24,976,1488);}
   disk(130,142,d.cost,'#62dce9');disk(894,142,d.power,'#d8ae5d');ctx.textAlign='center';ctx.textBaseline='alphabetic';ctx.fillStyle='#fff0bd';ctx.font='bold 52px Georgia';ctx.fillText(d.name,512,118,610);ctx.fillStyle='#d9c78e';ctx.font='bold 31px sans-serif';ctx.fillText((d.type||'CARTA').toUpperCase(),512,872,680);ctx.fillStyle='#17120d';ctx.font='32px Georgia';ctx.textAlign='center';wrap(d.text,512,980,724,43,1260);
  }texture.needsUpdate=true;}
 if(frame){frame.onload=paint;frame.src=new URL('../moldura.png',import.meta.url).href;}
 function makeButton(action){const c=document.createElement('canvas');c.width=512;c.height=128;const x=c.getContext('2d'),t=new THREE.CanvasTexture(c);t.colorSpace=THREE.SRGBColorSpace;const m=new THREE.Mesh(new THREE.PlaneGeometry(.29,.09),new THREE.MeshBasicMaterial({map:t,transparent:true,side:THREE.DoubleSide,depthTest:false,depthWrite:false}));m.position.z=.02;m.renderOrder=210;m.userData={kind:controlKind,id:action,action,cardId:null,enabled:false};group.add(m);function set(label,enabled=true,nextAction=action){x.clearRect(0,0,512,128);x.fillStyle=enabled?'#173b43':'#333b3e';x.fillRect(0,0,512,128);x.strokeStyle=enabled?'#72e5ed':'#777';x.lineWidth=10;x.strokeRect(5,5,502,118);x.fillStyle=enabled?'#fff0bd':'#aaa';x.font='bold 42px sans-serif';x.textAlign='center';x.textBaseline='middle';x.fillText(label,256,66,470);t.needsUpdate=true;m.userData={kind:controlKind,id:nextAction,action:nextAction,cardId:definition?.id,enabled};}return {mesh:m,set};}
 const activate=makeButton('activate-card'),utility=makeButton('card-utility');activate.mesh.position.set(-.16,-.43,.02);utility.mesh.position.set(.16,-.43,.02);
 function hide(){group.visible=mesh.visible=false;definition=null;activate.mesh.visible=utility.mesh.visible=false;}
 function show(d,actions=null){if(!d){hide();return;}definition=d;group.visible=mesh.visible=true;loadArt(d.key);paint();if(Array.isArray(actions)){const [first,second]=actions;activate.mesh.visible=!!first;utility.mesh.visible=!!second;if(first)activate.set(first.label,first.enabled!==false,first.action);if(second)utility.set(second.label,second.enabled!==false,second.action);}else{const activation=d.activation;activate.mesh.visible=!!activation;if(activation)activate.set(activation.label,activation.enabled);const utilityAction=d.pickup?'pickup':d.movable?'move':null;utility.mesh.visible=!!utilityAction;if(utilityAction)utility.set(utilityAction==='pickup'?'À MÃO':'MOVER',true,utilityAction);}if(activate.mesh.visible&&!utility.mesh.visible)activate.mesh.position.x=0;else activate.mesh.position.x=-.16;if(utility.mesh.visible&&!activate.mesh.visible)utility.mesh.position.x=0;else utility.mesh.position.x=.16;}
 function refresh(state){if(!definition)return;const next=state.cards.find(card=>card.id===definition.id);if(next)show(next);else hide();}
 return {group,mesh,targets:[activate.mesh,utility.mesh],show,hide,refresh,get visible(){return group.visible;},get cardId(){return definition?.id??null;}};
}
