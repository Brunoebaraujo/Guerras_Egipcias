...
export function createInspection(parent){
 const group=new THREE.Group();parent.add(group);group.position.set(0,.72,-.08);group.visible=false;
 ...
 function show(d){if(!d){hide();return;}definition=d;group.visible=true;loadArt(d.key);paint();const activation=d.activation;activate.mesh.visible=!!activation;if(activation)activate.set(activation.label,activation.enabled);const utilityAction=d.pickup?'pickup':d.movable?'move':null;utility.mesh.visible=!!utilityAction;if(utilityAction)utility.set(utilityAction==='pickup'?'À MÃO':'MOVER',true);utility.mesh.userData.action=utilityAction;utility.mesh.userData.cardId=d.id;if(activate.mesh.visible&&!utility.mesh.visible)activate.mesh.position.x=0;else activate.mesh.position.x=-.16;if(utility.mesh.visible&&!activate.mesh.visible)utility.mesh.position.x=0;else utility.mesh.position.x=.16;}
